#!/usr/bin/env python3
"""
Fine‑tunes Geneformer v1 and saves model after each epoch
"""
...
def build_geneformer_model(batch_size: int, n_classes: int):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[build_model] Using device: {device}")
    # config = GeneformerConfig(model_name="gf-12L-38M-i4096-CLcancer", batch_size=batch_size, device=device)
    config = GeneformerConfig(model_name="gf-6L-10M-i2048", batch_size=batch_size, device=device)
    model = GeneformerFineTuningModel(config, fine_tuning_head="classification", output_size=2)
    # model.load_state_dict(torch.load(r"C:\Users\james\scRNA\scRNArena\finetune\geneformer\checkpoints\pytorch_model_epoch3.bin", map_location=device))
    model.model.to(device).eval()
    return model
# ───────────────────────────────────────────────────────────────────────────────
# Metric evaluation helper (loss, accuracy, precision)
# ───────────────────────────────────────────────────────────────────────────────
def evaluateGF(model, ds, n_classes, batch_size=64, device="cuda"):
    """Evaluate model on a dataset and return loss, accuracy, precision, recall, and f1."""
    ce = torch.nn.CrossEntropyLoss(reduction="sum")
    logits_accum, labels_accum, total_loss = [], [], 0.0

    for i in range(0, len(ds), batch_size):
        batch = ds.select(range(i, min(i + batch_size, len(ds))))
        with torch.no_grad():
            logits = model.get_outputs(batch)
        y_true = torch.tensor(batch["label"], device=device)
        total_loss += ce(torch.tensor(logits, device=device), y_true).item()
        logits_accum.append(logits)
        labels_accum.append(batch["label"])

    logits_all = np.vstack(logits_accum)
    labels_all = np.concatenate(labels_accum)
...
def outputFMResults(model, tr_ds_fold, y_train_fold, val_ds_fold, y_val_fold, fold, epoch, n_classes, batch_size, device, all_results, out_dir):
    ...
    tr_loss, tr_acc, tr_prec, tr_rec, tr_f1 = evaluateGF(model, tr_ds_fold, n_classes, batch_size, device)
    val_loss, val_acc, val_prec, val_rec, val_f1 = evaluateGF(model, val_ds_fold, n_classes, batch_size, device)
...
def main():
    ...
    model = build_geneformer_model(batch_size, 2)

    tr_ds = model.process_data(ad_train, gene_names="gene_names").add_column("label", y_train)
    val_ds = model.process_data(ad_val, gene_names="gene_names").add_column("label", y_val)
